...
export namespace Git
{
    ...
    export async function getLastCommitInfo(repositoryPath: string, branch: string, file?: string): Promise<Commit>
    {
        ...
        const info = await Promise.all([
            Promisify.execPromise(`git log --pretty=format:'%H' -1 ${branch} ${tail}`, {cwd: repositoryPath}),
            Promisify.execPromise(`git log --pretty=format:'%cn' -1 ${branch} ${tail}`, {cwd: repositoryPath}),
            Promisify.execPromise(`git log --pretty=format:'%ce' -1 ${branch} ${tail}`, {cwd: repositoryPath}),
            Promisify.execPromise(`git log --pretty=format:'%cr' -1 ${branch} ${tail}`, {cwd: repositoryPath}),
            Promisify.execPromise(`git log --pretty=format:'%s' -1 ${branch} ${tail}`, {cwd: repositoryPath}),
        ]) as Array<string>;
    ...
    {
        const stdout = await Promisify.execPromise(`git ls-tree ${branch} ${path}`, {cwd: repositoryPath}) as string;
